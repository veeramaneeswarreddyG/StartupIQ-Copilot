...
import { StartupAnalysis } from "./src/types";
...
const generateLabel = (user_input: string, ai_response: string): string => {
  const input = user_input.toLowerCase();
  if (input.includes('improve') || input.includes('better')) return 'Improved Version';
  if (input.includes('add') || input.includes('include')) return 'Added Features';
  if (input.includes('change') || input.includes('modify')) return 'Modified Idea';
  if (input.includes('refine')) return 'Refined Concept';
  if (input.includes('scale') || input.includes('grow')) return 'Scalability Focus';
  if (input.includes('monetize') || input.includes('revenue')) return 'Monetization Update';
  if (input.includes('risk') || input.includes('problem')) return 'Risk Mitigation';
  return 'Updated Idea';
};
...
const getUserByToken = (token: string | undefined, db: DBSchema) => {
  if (!token || !mockTokens.has(token)) return null;
  const email = mockTokens.get(token)!;
  return db.users.find((u) => u.email === email) || null;
};

function createEmptyChat(type: "analysis" | "buddi", title: string): ChatEntry {
...
app.post("/api/chats", (req, res) => {
  ...
  const { title, type } = req.body as { title?: string; type?: "analysis" | "buddi" };
  if (!title || !type) {
...
app.post("/api/chats/:chatId/revisions", (req, res) => {
  ...
  const { user_input, ai_response, structured_output } = req.body as { user_input: string; ai_response: string; structured_output?: StartupAnalysis };
...
app.post("/api/analyze", async (req, res) => {
  const { idea, history } = req.body as {
    idea?: string;
  ...
  try {
    ...
    const analysis = JSON.parse(response.text) as StartupAnalysis;
